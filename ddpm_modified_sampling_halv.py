...
from diffusion import Skewed_GaussianDiffusionTrainer, GaussianDiffusionSampler
...
flags.DEFINE_integer('maxf', 100, help='max high frequency time step')
...
# Logging & Sampling
flags.DEFINE_string('logdir', '/ssd_scratch/cvit/Sai_niranjan/cifar_mnist_ddpm_skewed_samp', help='log directory')
...
def train():
    ...
    dataset_2 = MNIST(root='./data', train=True, download=True,
        transform=transforms.Compose([
            transforms.RandomHorizontalFlip(),
    ...
    dataset_2 = torch.utils.data.Subset(dataset_2, list(range(0, len(dataset_2), 2)))
    dataset_res = torch.utils.data.ConcatDataset([dataset,dataset_2])
    dataloader = torch.utils.data.DataLoader(
        dataset_res, batch_size=FLAGS.batch_size, shuffle=True,
    ...
    trainer = Skewed_GaussianDiffusionTrainer(
        net_model, FLAGS.beta_1, FLAGS.beta_T, FLAGS.T,FLAGS.maxf).to(device)
